...
export const x = 10;`
    ...
    describe("tsserverProjectSystem ProjectLoadingStart and ProjectLoadingFinish events", () => {
        ...
        const files = [libFile, aTs, configA, bTs, configB];
        ...
        function createSessionWithEventHandler(files: ReadonlyArray<File>) {
            ...
            host.readFile = file => {
                if (file === configA.path || file === configB.path) {
            ...
            return { host, session, verifyEvent, service: session.getProjectService() };
        ...
        function createSessionAndOpenATs(files: ReadonlyArray<File>) {
            const { host, session, service, verifyEvent } = createSessionWithEventHandler(files);

            openFilesForSession([aTs], session);
            checkNumberOfProjects(service, { configuredProjects: 1 });
            const project = service.configuredProjects.get(configA.path)!;
            assert.isDefined(project);
            verifyEvent(project, `Creating possible configured project for ${aTs.path} to open`);

            return { host, session, service, verifyEvent, project };
        }

        it("when project is created by open file", () => {
            const { session, service, verifyEvent } = createSessionAndOpenATs(files);

            openFilesForSession([bTs], session);
            checkNumberOfProjects(service, { configuredProjects: 2 });
            const projectB = service.configuredProjects.get(configB.path)!;
            assert.isDefined(projectB);
            verifyEvent(projectB, `Creating possible configured project for ${bTs.path} to open`);
        });

        it("when change is detected in the config file", () => {
            const {host, verifyEvent, project } = createSessionAndOpenATs(files);
